...
const Game = (() => {
  ...
  function buildEndingFootnote(flags, player) {
    ...
    if (player.track === 'court') {
      if ((f.loyal || 0) >= 3 && (f.judge || 0) >= 1) return '史家评曰："此人忠直廉明，身处浊世而不染，千古罕见。"';
      if ((f.corrupt || 0) >= 2 || (f.extort_used || 0) >= 4) return '野史记载："此人颇有手腕，私下敛财不少，然于大局无碍，世人褒贬各半。"';
      if ((f.loyal || 0) >= 2) return '同僚皆云："此人忠正，虽仕途多舛，然始终未曾弯腰媚上，难能可贵。"';
      if ((f.factioner || 0) >= 2) return '政敌称其："深于谋略，善于经营人脉，是个可怕的对手。"';
    }
    if (player.track === 'rebel') {
      if ((f.righteous || 0) >= 3) return '民间歌谣传唱："将军仁义，爱民如子，天下若有更多此等英雄，百姓何苦流离。"';
      if ((f.ruthless || 0) >= 2) return '史官秉笔："此人雄才大略，然手段刚烈，所过之处，民心难附，终成大业亦留骂名。"';
      if ((f.cunning || 0) >= 2) return '谋士赞曰："主公用兵如神，以奇谋破强敌，实乃百年难遇之帅才。"';
    }
    if (player.track === 'merchant') {
      if ((f.righteous || 0) >= 2) return '商界口碑："此人富而不骄，取之有道，是难得的商界清流，令人钦佩。"';
      if ((f.cunning || 0) >= 2) return '竞争对手忆述："此人精于算计，每一笔买卖皆算到极致，与之为敌实属不智。"';
      if ((f.royal_merchant || 0) >= 1) return '朝廷档案载："皇商某氏，为朝廷财政贡献颇巨，赏赐无数，亦是一段佳话。"';
    }
    if (player.track === 'hero') {
      if ((f.sworn || 0) >= 1 && (f.righteous || 0) >= 2) return '江湖传说："义字当头，兄弟情深——此人的名字，是江湖上最值得信任的三个字。"';
      if ((f.brave || 0) >= 2) return '武林人评："此人胆气无双，从未见过他退缩，无论对手多强，皆敢正面迎战。"';
      if ((f.lone_hero || 0) >= 1) return '过客忆述："曾见一白衣剑客，孤身行走天涯，不问名利，只问公道——那是真正的侠客。"';
    }
    return null; // 无特别注脚
  }
